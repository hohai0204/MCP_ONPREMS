*chạy se37 tạo 1 function group xong active function group, xong bỏ 1 function module tên như dưới(zmcp_adt_ddic_doma) vào function group*
...
*sau đó. check đã tạo thành công chưa*
*sau đó bash code dưới đây vào zmcp_adt_ddic_doma*
* ZMCP_ADT_DDIC_DOMA - DDIC domain create/update/delete over RFC.
* Adapted from superclaude-for-sap (MIT, (c) paek seunghyun) for the
* MCP_SAP_PRIVATE RFC-only bridge. Install in function group
* ZMCP_ADT_UTILS and flag REMOTE-ENABLED (SE37 > Attributes).
* DDIF_* APIs are release-stable; used here on S/4HANA (DS4) because
* this bridge has no ADT/HTTP access.
*
* Actions:
